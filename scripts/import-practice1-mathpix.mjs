...
const TEX_PATH = path.join(
  ROOT,
  '84d71dbd-4229-4b23-8e15-20daa363abf6',
  '84d71dbd-4229-4b23-8e15-20daa363abf6.tex',
);
const IMAGE_SOURCE_DIR = path.join(ROOT, '84d71dbd-4229-4b23-8e15-20daa363abf6', 'images');
const IMAGE_TARGET_DIR = path.join(ROOT, 'public', 'question-assets', 'ets-form-gr1268');
const OUTPUT_JSON_PATH = path.join(ROOT, 'src', 'data', 'banks', 'ets-form-gr1268.json');
const BANK_ID = 'ets-form-gr1268';
const SOURCE_NOTE = 'ETS GRE Mathematics Test Practice Book, Form GR1268';
...
function normalizeLatexEnvironments(text) {
  return text.replace(
    /\\begin\{(array|aligned|gathered)\}(\{[^}]+\})?([\s\S]*?)\\end\{\1\}/g,
...
function convertTexToMarkdown(raw, imageMap) {
  ...
  text = text
    ...
    .replace(/\\includegraphics\[[^\]]*]\{([^}]+)\}/g, (_, imageName) => {
      ...
      return `![](/question-assets/ets-form-gr1268/${resolvedFile})`;
    })
    .replace(/\\\\/g, '\n\n');
...
function parseQuestionBodies(tex) {
  const questionSection = tex.split('\\section*{SUBJECT TEST}')[0];
...
function splitStemAndOptions(questionBody) {
  const optionMatches = [...questionBody.matchAll(/(?:^|\n)[ \t]*\(([A-E])\)\s*/g)].map(
    (match) => ({
    label: match[1],
      index:
        match.index +
  ...
  if (optionMatches.length < 5) {
    throw new Error(`Question body is missing option labels:\n${questionBody.slice(0, 200)}`);
  }

  const stem = questionBody.slice(0, optionMatches[0].index).trim();
  const options = optionMatches.slice(0, 5).map((match, index) => {
    const start = match.index + match.markerLength;
    const end =
      index + 1 < optionMatches.length ? optionMatches[index + 1].index : questionBody.length;
    return questionBody.slice(start, end).trim();
  });
...
async function main() {
  ...
  const questions = questionBodies.map((body, index) => {
    ...
    return {
      ...
      solution_markdown: `Official answer key: ${correctAnswer}. Worked solutions are not included in the source practice book.`,
      source_note: SOURCE_NOTE,
      tags: ['official', 'ets', 'practice-book', 'form-gr1268'],
    };
  });

  moveTrailingBlockToNextStem(questions, 7, 'image');
  moveTrailingBlockToNextStem(questions, 10, 'math');
  moveTrailingBlockToNextStem(questions, 25, 'image');
  moveTrailingBlockToNextStem(questions, 26, 'math');
  moveTrailingBlockToNextStem(questions, 44, 'math');

  const bank = {
    id: BANK_ID,
    title: 'ETS GRE Mathematics Test Practice Form GR1268',
    description:
      'Imported from the official ETS GRE Mathematics Test Practice Book (Form GR1268).',
    questions,
  };
